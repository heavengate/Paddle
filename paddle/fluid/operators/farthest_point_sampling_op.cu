...
class FarthestPointSamplingOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    // allocate memory
    output->mutable_data<int>(ctx.GetPlace());
    ...
    // faltten
    auto in_points = framework::EigenVector<T>::Flatten(*input);
    const T *ptr_in_points = &(in_points(0));

    Tensor *tmp;
    tmp->mutable_data<T>(framework::make_ddim({32, in_n_points}),
                         ctx.GetPlace());
    auto tmp_e = framework::EigenVector<T>::Flatten(*tmp);
    T *ptr_tmp_e = &(tmp_e(0));

    auto out_points_index = framework::EigenVector<int>::Flatten(*output);
    int *ptr_out_points_index = &(out_points_index(0));

    // run fathest point sampling
    farthestpointsamplingKernel<<<32, 512>>>(batch_size, in_n_points,
                                             out_m_points, ptr_in_points,
                                             ptr_tmp_e, ptr_out_points_index);
  }
};
